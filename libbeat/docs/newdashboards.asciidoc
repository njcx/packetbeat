...
https://packetbeat/tree/master/testing/environments[beats/testing/environments] to import, create, and
...
https://packetbeat/tree/master/libbeat/dashboards[beats/libbeat/dashboards]. It's also available in each Beat package under the `scripts` directory.
...
For development or community Beats, it's easier to run the `import_dashboards` script from the https://packetbeat/tree/master/libbeat/dashboards[beats/libbeat/dashboards] directory. In this case, you need to first compile the script:
...
https://packetbeat/tree/master/dev-tools[dev-tools]. See the dev-tools
https://packetbeat/tree/master/dev-tools/README.md[readme] for more info.
